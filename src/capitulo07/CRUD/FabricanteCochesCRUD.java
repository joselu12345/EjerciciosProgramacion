...
public class FabricanteCochesCRUD {
	...
	public static void crearFabricantes() throws SQLException {
		...
		Statement s = (Statement) conexion.createStatement();
		String sql = "insert into fabricante (id, CIF, nombre) values ( " + (id + 1) + ", '" + CIF
				+ "', '" + nombre + "')";
		s.executeUpdate(sql);
	...
	public static void cambiarFabricantes() throws SQLException {
		...
		Statement s = (Statement) conexion.createStatement();
		String sql = "update fabricante set cif= '" + CIF + "', nombre= '" + nombre + "' where id=" + id;
		s.executeUpdate(sql);
	...
	public static Connection getConexion() throws SQLException {
		// Si es la primera vez que accedemos a la conexion, debemos instanciarla
		if (conexion == null) {
			conectar();
		}
		// Compruebo si la conexion sigue estando activa
	...
	private static int maxIdEnTabla(Connection conn, String tabla) throws SQLException {
		...
		String sql = "select max(id) from fabricante" + tabla;
